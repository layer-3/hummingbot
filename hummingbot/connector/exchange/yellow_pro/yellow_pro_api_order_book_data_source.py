...
class YellowProAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        authenticator = self._connector.authenticator
        if authenticator is not None:
            token = await authenticator.ensure_active_token()
            if token:
        ...
        try:
            ...
            if authenticator is not None:
                try:
                    ...
                    self.logger().debug("Failed to obtain YellowPro token for public websocket connect.", exc_info=True)
            connect_payload: Dict[str, Any] = {
                "id": next(self._subscription_id_iterator),
                "connect": {},
            }
            if token:
                connect_payload["connect"]["token"] = token
